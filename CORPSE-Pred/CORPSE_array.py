expected_params={	'vmaxref': 'Relative maximum enzymatic decomp rates (length 3)',
            ...
            'new_resp_units':True,
            'vmaxref_predator':'Maximum grazing rate',
            'Ea_predator':'Activation energy for predators',
            'minPredatorC':	   'Minimum predator biomass (fraction of microbe C)',
            'Tpredator': 'Predator lifetime (years)',
            'et_predator':  'Fraction of predator turnover not converted to CO2',
            'eup_predator': 'CUE for predators',
            'kC_predator':'Half saturation constant for predators'
            }

chem_types = ['Fast','Slow','Necro']

expected_pools = ['u'+t for t in chem_types]+\
                 ['p'+t for t in chem_types]+\
                 ['livingMicrobeC','predatorC','CO2','originalC']
...
def CORPSE_deriv(SOM,T,theta,params,claymod=1.0):
    ...
    predatorTurnover=(SOM['predatorC']-params['minPredatorC']*SOM['livingMicrobeC'])/params['Tpredator']
    if isinstance(predatorTurnover,float):
        predatorTurnover=max(0.0,predatorTurnover)
    else:
        predatorTurnover[predatorTurnover<0.0]=0.0

    predator_resp=predatorTurnover*(1.0-params['et_predator'])

    deadpredator_C_production=predatorTurnover*et


    # CO2 production and cumulative CO2 produced by cohort
    CO2prod=maintenance_resp+predator_resp
    for t in chem_types:
        CO2prod=CO2prod+decomp[t]*(1.0-eup[t])

    microbeGrowth=zeros_like(CO2prod)
    for t in chem_types:
        microbeGrowth=microbeGrowth+decomp[t]*eup[t]

    predatorGrowth=predation_rate*params['eup_predator']
    CO2prod=CO2prod+predation_rate*(1.0-params['eup_predator'])
    ...
    for k in derivs.keys():
        derivs[k]=0.0
    derivs['livingMicrobeC']=microbeGrowth-microbeTurnover-predation_rate
    derivs['CO2']=CO2prod
    derivs['originalC']=zeros_like(CO2prod)
    derivs['predatorC']=predatorGrowth-predatorTurnover

    for t in chem_types:
        derivs['u'+t+'C']=-decomp[t]+protectedCturnover[t]-protectedCprod[t]
        derivs['p'+t+'C']=protectedCprod[t]-protectedCturnover[t]

    derivs['uNecroC']=derivs['uNecroC']+deadmic_C_production+deadpredator_C_production
    return derivs
...
def Vmax_predator(T,params):
    ...
    Vmax=params['vmaxref_predator']*exp(-params['Ea_predator']*(1.0/(Rugas*T)-1.0/(Rugas*Tref)));
    return Vmax
