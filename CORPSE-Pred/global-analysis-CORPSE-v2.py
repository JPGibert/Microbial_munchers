...
def run_array(SOM_init,params,nyears,forcing,inputs,claydata,do_RK=False,output_yrs=1):
    ...
    for step in range(nsteps):
        ...
        if do_RK:
            RK=rungekutta(SOM,CORPSE_array.CORPSE_deriv,dt,Ts_now,Theta_now,params,claymod=claymod)
            for pool in SOM.keys():
                SOM[pool]=RK[pool]
        else:
            derivs=CORPSE_array.CORPSE_deriv(SOM,Ts_now,Theta_now,params,claymod=claymod)
...
# This is the same as the ratio between the ideal gas constant R=8.314472 J/K/mol and the Bolzmann constant 8.62e-5 eV/K
    ...
    pred_Ea30=run_array('CORPSE_pred_Ea30_051418.nc',params,nyears,forcing,inputs,claymap,do_RK=False,output_yrs=5)
    pred_Ea30_warmed=run_array('CORPSE_pred_Ea30_051418.nc',params,nyears,forcing_warmed,inputs,claymap,do_RK=False,output_yrs=5)
    ...
    pred_Ea10=run_array('CORPSE_pred_Ea10_051418.nc',params_pred_Ea10,nyears,forcing,inputs,claymap,do_RK=False,output_yrs=5)
    pred_Ea10_warmed=run_array('CORPSE_pred_Ea10_051418.nc',params_pred_Ea10,nyears,forcing_warmed,inputs,claymap,do_RK=False,output_yrs=5)
    ...
    contourf(lon,nopred.lat,mapdata,cmap='BrBG_r',levels=arange(-8,9,1)*2,extend='both')
    gca().coastlines()
    cb=colorbar()
    cb.set_label('kg C m$^{-2}$')
    title('Total C stock difference from no-pred')
    letter_label(letter=ascii_lowercase[5-1])
    ...
    cb.set_label('kg C m$^{-2}$')
    title('Total C stock difference from no-pred')
    letter_label(letter=ascii_lowercase[6-1])
    gl=ax.gridlines(draw_labels=True,ylocs=ygrids,xlocs=xgrids)
    gl.ylabels_right=False
    gl.xlines=False
    gl.xlabels_top=False;gl.xlabels_bottom=False
    ...
    ax=subplot(322,projection=ccrs.PlateCarree())
    mapdata=add_cyclic_point(pred_Ea10['predatorC'].isel(time=-1).values)
    contourf(lon,nopred.lat,mapdata*1e3,cmap='magma_r',levels=arange(0,31,2.5)*2,extend='max')
    gca().coastlines()
    cb=colorbar()
    cb.set_label('g C m$^{-2}$')
    title('Low T sensitivity:\nTotal predator biomass C')
    ...
    # mapdata=add_cyclic_point(pred_Ea10['livingMicrobeC'].isel(time=-1).values-nopred['livingMicrobeC'].isel(time=-1).values)
    mapdata=add_cyclic_point(micfrac(pred_Ea10).values/micfrac(nopred).values-1)*100
    contourf(lon,nopred.lat,mapdata,cmap='BrBG_r',levels=arange(-50,51,5)*2,extend='both')
    gca().coastlines()
    cb=colorbar()
    cb.set_label('% Difference from no-pred')
    title('% Difference in microbial biomass fraction')
    letter_label(letter=ascii_lowercase[4-1])
    gl=ax.gridlines(draw_labels=True,ylocs=ygrids,xlocs=xgrids)
    gl.ylabels_right=False
    gl.xlines=False
    gl.xlabels_top=False;gl.xlabels_bottom=False
    ...
    plot(lat,totalCarbon(pred_Ea10).mean(dim=('lon')).isel(time=-1),c='C2',ls='-',lw=1.0,label='Pred (Ea 10)')
    plot(lat,totalCarbon(pred_Ea10).mean(dim=('lon')).isel(time=1),c='C2',ls='--',lw=1.0)
    legend()
    subplot(312)
    for n,lt in enumerate([30,45,62,73]):

        plot(totalCarbon(pred_Ea30).isel(lat=lt).mean(dim='lon')[1:],c='C%d'%n,lw=1.0,label='Lat = %1.1f'%pred_Ea30.lat[lt])
        plot(totalCarbon(nopred).isel(lat=lt).mean(dim='lon')[1:],ls='--',c='C%d'%n,lw=1.0)
        plot(totalCarbon(pred_Ea10).isel(lat=lt).mean(dim='lon')[1:],ls=':',c='C%d'%n,lw=1.0)
    legend()
    subplot(313)
    ...
    plot((totalCarbon(pred_Ea10)*cell_area).sum(skipna=True,dim=('lat','lon'))[1:]*1e-12,c='C2',ls='-')
    ...
    plot((totalCarbon(pred_Ea10_warmed)*cell_area).sum(skipna=True,dim=('lat','lon'))[1:]*1e-12,c='C2',ls='--')
    ...
    figure(7)
    plot_equils(pred_Ea10.isel(time=-1))
